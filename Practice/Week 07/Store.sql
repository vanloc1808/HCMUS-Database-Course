...
ALTER TABLE CLASS ADD Evaluation NVARCHAR(10)
GO

ALTER PROC CSDL_RESULT
	@CLASSID NCHAR(10)
AS
BEGIN
	IF ((SELECT COUNT(*)
	FROM CLASS AS C
	WHERE C.ID = @CLASSID) = 0)
	BEGIN
	...
	BEGIN
		...
		DECLARE @CSDL_ID NCHAR(10)
		SET @CSDL_ID =
			(
				SELECT S.ID
				FROM Subject AS S
				WHERE S.Name = N'Cơ sở dữ liệu'
			)
		SET @PASS_NUMBER = 
			(
				SELECT COUNT(*)
				FROM Course AS CO, Student AS ST, Subject AS SU, Result AS RE
				WHERE CO.ClassID = ST.ClassID AND CO.ClassID = @CLASSID AND CO.SubjectID = SU.ID
				AND SU.ID = @CSDL_ID
				AND RE.StudentID = ST.ID AND RE.SubjectID = SU.ID AND RE.Mark >= 5
				AND RE.Times >= ALL
					(
						SELECT R1.Times
						FROM RESULT AS R1
						WHERE R1.StudentID = RE.StudentID AND R1.SubjectID = RE.SubjectID
					)
			)
		...
		IF (2 * @PASS_NUMBER < @STUDENT_NUMBER)
		BEGIN
			UPDATE CLASS 
...
EXEC CSDL_RESULT 'LH000001'
GO

CREATE PROC EVAL_ALL
AS
BEGIN
